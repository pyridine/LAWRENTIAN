...
class diff_match_patch {
  ...
  */
  class Diff {
   public:
     ...
     */
    Diff(D_Operation _operation, const string_t &_text) : operation(_operation), text(_text) {}
    Diff() {}
    ...
    static string_t strOperation(D_Operation op) {
      switch (op) {
        case D_INSERT:
 ...
 private:
  static void diff_main(const string_t &text1, const string_t &text2, bool checklines, clock_t deadline, Diffs& diffs) {
    ...
    if (text1 == text2) {
      if (!text1.empty()) {
        diffs.push_back(Diff(D_EQUAL, text1));
      }
    }
    else {
      ...
      if (!commonprefix.empty()) {
        diffs.push_front(Diff(D_EQUAL, commonprefix));
      }
      if (!commonsuffix.empty()) {
        diffs.push_back(Diff(D_EQUAL, commonsuffix));
      }
 ...
 private:
  static void diff_compute(string_t text1, string_t text2, bool checklines, clock_t deadline, Diffs& diffs) {
    if (text1.empty()) {
      ...
      diffs.push_back(Diff(D_INSERT, text2));
      return;
    }

    if (text2.empty()) {
      // Just delete some text (speedup).
      diffs.push_back(Diff(D_DELETE, text1));
      return;
    }

    {
      ...
      if (i != string_t::npos) {
        ...
        const D_Operation op = (text1.length() > text2.length()) ? D_DELETE : D_INSERT;
        diffs.push_back(Diff(op, longtext.substr(0, i)));
        diffs.push_back(Diff(D_EQUAL, shorttext));
    ...
    if (deadline != std::numeric_limits<clock_t>::max()) {
      ...
      if (diff_halfMatch(text1, text2, hm)) {
        ...
        diffs.push_back(Diff(D_EQUAL, hm.mid_common));
        Diffs diffs_b;
 ...
 private:
  static void diff_lineMode(string_t text1, string_t text2, clock_t deadline, Diffs& diffs) {
    ...
    diffs.push_back(Diff(D_EQUAL, string_t()));
    int count_delete = 0;
    int count_insert = 0;
  ...
  static void diff_cleanupSemantic(Diffs &diffs) {
    ...
    for (cur_diff = diffs.begin(); cur_diff != diffs.end();) {
      if ((*cur_diff).operation == D_EQUAL) {
        ...
        if ((*cur_diff).operation == D_INSERT) {
        ...
        if (!lastequality.empty()
          ...
          (*(cur_diff = equalities.back())).operation = D_INSERT;
          // Duplicate record.
          diffs.insert(cur_diff, Diff(D_DELETE, lastequality));
    ...
    if ((cur_diff = diffs.begin()) != diffs.end()) {
      for (typename Diffs::iterator prev_diff = cur_diff; ++cur_diff != diffs.end(); prev_diff = cur_diff) {
        if ((*prev_diff).operation == D_DELETE &&
            (*cur_diff).operation == D_INSERT) {
          ...
          if (overlap_length != 0) {
            ...
            diffs.insert(cur_diff, Diff(D_EQUAL, insertion.substr(0, overlap_length)));
            (*prev_diff).text =
 ...
 public:
  static void diff_cleanupSemanticLossless(Diffs &diffs) {
    ...
    for (typename Diffs::iterator next_diff = cur_diff; ++next_diff != diffs.end(); prev_diff = cur_diff, cur_diff = next_diff) {
      if ((*prev_diff).operation == D_EQUAL &&
        (*next_diff).operation == D_EQUAL) {
  ...
  void diff_cleanupEfficiency(Diffs &diffs) const {
    ...
    for (typename Diffs::iterator cur_diff = diffs.begin(); cur_diff != diffs.end();) {
      if ((*cur_diff).operation == D_EQUAL) {
        ...
        // An insertion or deletion.
        if ((*cur_diff).operation == D_DELETE) {
          post_del = true;
        } else {
          post_ins = true;
        }
 ...
 public:
  static void diff_cleanupMerge(Diffs &diffs) {
    diffs.push_back(Diff(D_EQUAL, string_t()));  // Add a dummy entry at the end.
        ...
        case D_EQUAL:
          if (count_delete + count_insert > 1) {
            ...
            if (count_delete != 0 && count_insert != 0) {
              ...
              if (commonlength != 0) {
                if (cur_diff != diffs.begin()) {
                  ...
                  diffs.insert(cur_diff, Diff(D_EQUAL, text_insert.substr(0, commonlength)));
                }
            ...
            if (!text_delete.empty()) {
              diffs.insert(cur_diff, Diff(D_DELETE, text_delete));
            }
            if (!text_insert.empty()) {
              diffs.insert(cur_diff, Diff(D_INSERT, text_insert));
            }
 ...
 public:
  static int diff_xIndex(const Diffs &diffs, int loc) {
    ...
    if (last_diff != diffs.end() && (*last_diff).operation == D_DELETE) {
    ...
    for (typename string_t::const_pointer token = delta.c_str(); token - delta.c_str() < (int)delta.length(); token += token_len + 1) {
      ...
      switch (traits::to_wchar(*token)) {
        case L'+':
          ...
          diffs.push_back(Diff(D_INSERT, param));
          break;
        case L'-':
          // Fall through.
        case L'=': {
          ...
          if (traits::to_wchar(*token) == L'=') {
            diffs.push_back(Diff(D_EQUAL, text));
          } else {
            diffs.push_back(Diff(D_DELETE, text));
          }
          break;
        }
        default:
 ...
 protected:
  void patch_addContext(Patch &patch, const string_t &text) const {
    ...
    if (!prefix.empty()) {
      patch.diffs.push_front(Diff(D_EQUAL, prefix));
    }
    // Add the suffix.
    ...
    if (!suffix.empty()) {
      patch.diffs.push_back(Diff(D_EQUAL, suffix));
    }
 ...
 public:
  Patches patch_make(const string_t &text1, const Diffs &diffs) const {
    ...
    if (!diffs.empty()) { // Get rid of the null case.
      ...
      for (typename Diffs::const_iterator cur_diff = diffs.begin(); cur_diff != diffs.end(); ++cur_diff) {
        if (patch.diffs.empty() && (*cur_diff).operation != D_EQUAL) {
          ...
          if ((int)text1.length() > Match_MaxBits
            ...
            for (typename Diffs::const_iterator cur_diff = (*cur_patch).diffs.begin(); cur_diff != (*cur_patch).diffs.end(); ++cur_diff) {
              if ((*cur_diff).operation != D_EQUAL) {
                ...
                } else if ((*cur_diff).operation == D_DELETE) {
                  // Deletion
 ...
 public:
  string_t patch_addPadding(Patches &patches) const {
    ...
    if (firstPatchDiffs.empty() || firstPatchDiffs.front().operation != D_EQUAL) {
      // Add nullPadding equality.
      firstPatchDiffs.push_front(Diff(D_EQUAL, nullPadding));
    ...
    if (lastPatchDiffs.empty() || lastPatchDiffs.back().operation != D_EQUAL) {
      // Add nullPadding equality.
      lastPatchDiffs.push_back(Diff(D_EQUAL, nullPadding));
      ...
      while (!bigpatch.diffs.empty()) {
        ...
        while (!bigpatch.diffs.empty()
          ...
          if (diff_type == D_INSERT) {
            ...
            empty = false;
          } else if (diff_type == D_DELETE && patch.diffs.size() == 1
              && patch.diffs.front().operation == D_EQUAL
        ...
        if (!postcontext.empty()) {
          ...
          if (!patch.diffs.empty()
              && patch.diffs.back().operation == D_EQUAL) {
            patch.diffs.back().text += postcontext;
          } else {
            patch.diffs.push_back(Diff(D_EQUAL, postcontext));
          }
        }
        if (!empty) {
      ...
      while (text - textline.c_str() < (int)textline.length()) {
        ...
        for (text += text_len + 1; text - textline.c_str() < (int)textline.length(); text += text_len + 1) {
          ...
          switch (traits::to_wchar(sign)) {
            case L'-':
              // Deletion.
              patch.diffs.push_back(Diff(D_DELETE, line));
              continue;
            case L'+':
              // Insertion.
              patch.diffs.push_back(Diff(D_INSERT, line));
              continue;
            case L' ':
              // Minor equality.
              patch.diffs.push_back(Diff(D_EQUAL, line));
              continue;
            case L'@':
              // Start of next patch.
              break;
